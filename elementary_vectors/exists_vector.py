...
def exists_vector(data, L, R, l=True, r=True, kernel=False, certificate=False):
    r"""
    Returns whether a vector exists in the space determined by a matrix such
    that the components lie in given intervals.
    ...
    - If ``data`` is a matrix, then the elementary vectors of ``M`` are computed.
        ...
        - If ``kernel`` is false (default), considers the vector space generated
          by the rows of the matrix ``M``. (In this case, the elementary vectors
          will be lie in the kernel of ``M``)
          
        - If ``kernel`` is true, considers the vector space generated by the
          kernel of the matrix ``M``. (In this case, the elementary vectors will
          be lie in the row space of ``M``)
